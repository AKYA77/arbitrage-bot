...
logger = logging.getLogger("bitget_wrapper")

class BitgetWrapper:
    def __init__(self):
        """
        Initialize the Bitget exchange wrapper with API credentials if available.
        Falls back to public API if no credentials are provided.
        """
        api_keys = load_api_keys().get("bitget", {})
        ...
        # WebSocket URLs
        self.ws_public_url = "wss://ws.bitget.com/spot/v1/stream"
    ...
    def get_order_book(self, symbol: str, depth: int = 5) -> Dict:
        ...
        try:
            ...
            raise Exception(f"Error fetching order book for {symbol} on Bitget: {str(e)}")
    ...
    def get_websocket_subscribe_message(self) -> str:
        ...
        pairs = ["BTCUSDT", "ETHUSDT", "ETHBTC", "LTCUSDT", "LTCBTC", "XRPUSDT", "XRPBTC"]
        
        # Format subscription message for Bitget
        channels = []
        for pair in pairs:
            channels.append(f"books_{pair}")
    ...
    def parse_websocket_message(self, message: str) -> List[Dict]:
        ...
        try:
            ...
            if 'data' in data and 'action' in data and data['action'] == 'update':
                if 'arg' in data and 'instId' in data['arg']:
                    ...
                    book_data = data['data'][0] if isinstance(data['data'], list) and data['data'] else data['data']
            ...
            logger.error(f"Error parsing Bitget WebSocket message: {str(e)}")
            return []

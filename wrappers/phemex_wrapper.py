...
logger = logging.getLogger("phemex_wrapper")

class PhemexWrapper:
    def __init__(self):
        """
        Initialize the Phemex exchange wrapper with API credentials if available.
        Falls back to public API if no credentials are provided.
        """
        api_keys = load_api_keys().get("phemex", {})
        ...
        # WebSocket URLs
        self.ws_public_url = "wss://phemex.com/ws"
        self.ws_private_url = "wss://phemex.com/ws"
    ...
    def get_order_book(self, symbol: str, depth: int = 5) -> Dict:
        ...
        try:
            ...
            raise Exception(f"Error fetching order book for {symbol} on Phemex: {str(e)}")
    ...
    def get_websocket_subscribe_message(self) -> str:
        ...
        # Return first subscription (will send others separately)
        return json.dumps(subscriptions[0])
    ...
    def parse_websocket_message(self, message: str) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Error parsing Phemex WebSocket message: {str(e)}")
            return []
